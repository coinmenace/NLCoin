...
- [Building Proton Core](#building-proton-core)
...
git clone https://github.com/nlcoin/protoncoin
```
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/proton/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/nlcoin/protoncoin
...
For example:
```bash
URL=https://github.com/crowning-/proton.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit proton=${COMMIT} --url proton=${URL} ../proton/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit proton=${COMMIT} --url proton=${URL} ../proton/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit proton=${COMMIT} --url proton=${URL} ../proton/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the proton git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../proton/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/nlcoin.git
SIGPATH=/some/root/path/nlcoin-detached-sigs.git

./bin/gbuild --url proton=${BTCPATH},signature=${SIGPATH} ../proton/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/proton-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/proton-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/proton-osx-build.assert
```
...
[proton/gitian.sigs](https://github.com/nlcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
